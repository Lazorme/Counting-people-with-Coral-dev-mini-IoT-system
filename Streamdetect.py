...
def main():
    ...
    while cap.isOpened():
        ...
        if button.read()==1:
            ...
            print(person_counter_enter)
            print(person_counter_leave)
            person_inside = person_counter_enter-person_counter_leave
            cv2.putText(cv2_im, f"Personnes enters: {person_counter_enter}", (10, 80), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
            cv2.putText(cv2_im, f"Personnes enters: {person_counter_enter}", (10, 70), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
            cv2.putText(cv2_im, f"Personnes leaves: {person_counter_leave}", (10, 50), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
